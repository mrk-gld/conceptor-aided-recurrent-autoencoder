...
def visualize_sine_interpolation(
    params, conceptors, log_folder, fname, len_seqs=300, ntype="rnn"
):
    """
    ...
    """
    forward_rnn_interp = (
        forward_rnn_interp_rnn if ntype == "rnn" else forward_rnn_interp_lstm
    )
    ...
    for idx, lamda in enumerate([0, 0.25, 0.5, 0.75, 1]):
        ...
        x_interp, y_interp = forward_rnn_interp(
            params, conceptors, None, ratio=lamda, length=len_seqs, spd_interp=None
        )
...
def visualize_mocap_interpolation(
    params, conceptors, log_folder, filename, ntype="rnn"
):
    """
    ...
    for lamda, state in zip(lamdas, states):
        ...
        ax.plot(
            data_pca[:, 0], data_pca[:, 1], lamda, label=r"$\lambda$={}".format(lamda)
        )
...
def compute_JS_divergence_and_acf(
    timeseries1, timeseries2, timeseries_interp, lag=10, bins=50
):
    ...
    min_len = np.min(
        [timeseries1.shape[0], timeseries2.shape[0], timeseries_interp.shape[0]]
    )
    ...
    for p in range(timeseries1.shape[1]):
        ...
        prob_interp = (
            np.histogram(timeseries_interp[:, p], bins=bins, density=True)[0] + 1e-18
        )
